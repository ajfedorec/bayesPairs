...
#'
#' @examples
bayesPairs <- function(posteriors, priors){
  ...
  for(i in 1:(n_params+1)){
    for(j in 1:(n_params+1)) local({
      ...
      if((i == 1) & (j == 1)) {
        ...
        ## plot top row rug plot
        plt <- ggplot2::ggplot() +
          # ggplot2::geom_density(data=posteriors, ggplot2::aes(posteriors[j-1]), size=0.2)+
        ...
        ## plot RHS rug plot
        plt <- ggplot2::ggplot() +
          # ggplot2::geom_density(data=posteriors, ggplot2::aes(posteriors[i-1]), size=0.2)+
        ...
        ## plot 2D density
        plt <- ggplot2::ggplot() +
          ggplot2::stat_density2d(ggplot2::aes(x = posteriors[j-1], y = posteriors[i-1],
                                               fill=..level..),
  ...
  return(plt.all)
  # ggplot2::ggsave("bayesPairs.pdf", plot = plt.all, width=20*(n_params+1), height=20*(n_params+1), units="mm")
}
